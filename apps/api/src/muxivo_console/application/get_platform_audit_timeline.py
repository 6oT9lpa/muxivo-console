"""Read a sanitized Activity audit timeline through one platform connection."""
...
class GetPlatformAuditTimeline:
    authorizer: OrganizationAuthorizer
    connections: PlatformConnectionReader
    readers: Mapping[Platform, PlatformAuditTimelineReader]
    ...
    async def execute(
        ...
        *,
        ...
        if not decision.allowed:
            raise AccessDeniedError("The actor is not allowed to view audit timeline events.")
        ...
        if reader is None:
            raise PlatformHealthUnavailableError("No audit timeline adapter is available.")
        return await reader.get_audit_timeline_for_connection(
            organization_id=organization_id,
            actor_id=actor_id,
