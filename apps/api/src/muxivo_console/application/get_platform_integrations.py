"""Read one connected platform's secret-free integration state."""
...
class GetPlatformIntegrations:
    authorizer: OrganizationAuthorizer
    connections: PlatformConnectionReader
    readers: Mapping[Platform, PlatformIntegrationsReader]
    ...
    async def execute(
        ...
        if not decision.allowed:
            raise AccessDeniedError("The actor is not allowed to view integrations.")
        ...
        if reader is None:
            raise PlatformHealthUnavailableError("No integrations adapter is available.")
        return await reader.get_integrations_for_connection(
