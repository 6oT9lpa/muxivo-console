"""Discord Control API adapter for browser-safe connection candidate discovery."""
...
class DiscordPlatformConnectionCandidateCatalog:
    """Ask Discord for resources the linked identity can administer."""
    ...
    async def list_for_organization(
        ...
        logger.info(
            "platform_connection_candidates.discord.started",
            extra={
        ...
        if platform_subject is None:
            logger.info(
                "platform_connection_candidates.discord.identity_missing",
                extra={
                    "actor_id": str(actor_id),
                    "organization_id": str(organization_id),
                    "correlation_id": str(correlation_id),
        ...
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.get(
                    ...
                    params={"platform": Platform.DISCORD.value},
            ...
            logger.warning(
                "platform_connection_candidates.discord.unavailable",
                extra={
                    "actor_id": str(actor_id),
                    "organization_id": str(organization_id),
                    "correlation_id": str(correlation_id),
            ...
            raise PlatformControlUnavailableError(
                "Discord Control API connection candidate discovery failed."
            ) from error
        catalog = _parse_catalog(payload)
        logger.info(
            "platform_connection_candidates.discord.completed",
            extra={
                "actor_id": str(actor_id),
                "organization_id": str(organization_id),
...
def _parse_catalog(payload: Any) -> PlatformConnectionCandidateCatalog:
    if not isinstance(payload, dict) or not isinstance(payload.get("items"), list):
        raise PlatformControlUnavailableError(
            "Discord Control API returned an invalid connection candidate payload."
        )
    try:
        ...
        raise PlatformControlUnavailableError(
            "Discord Control API returned invalid connection candidate values."
        ) from error
